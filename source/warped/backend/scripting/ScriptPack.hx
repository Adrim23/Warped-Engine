...
import warped.backend.scripting.events.CancellableEvent;
import backend.Mods;
...
class ScriptPack extends Script {
	...
	public function setStageSprites(mapSprites:Map<String, FlxSprite>)
		for (name in mapSprites.keys())
              set(name, mapSprites.get(name));
