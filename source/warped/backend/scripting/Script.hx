package warped.backend.scripting;
...
import warped.backend.scripting.HScriptCodename;
import flixel.ui.FlxBar;

@:allow(warped.backend.scripting.ScriptPack)
/**
...
class Script extends FlxBasic implements IFlxDestroyable {
	...
	public static function getDefaultVariables(?script:Script):Map<String, Dynamic> {
		return [
			...
			"FlxTrail"		  => warped.flixel.addons.effects.FlxTrail,
			"FlxVideo"		  => hxvlc.flixel.FlxVideo,
			"FlxVideoSprite"		  => hxvlc.flixel.FlxVideoSprite,

			// Engine related stuff
			"ModState"		  => warped.backend.scripting.ModState,
			"ModSubState"	   => warped.backend.scripting.ModSubState,
			...
			"HudCamera"		 => warped.objects.HudCamera,
			"Note"			  => objects.Note,
			...
			"Options"		   => warped.backend.options.Options,
			"Paths"			 => backend.Paths,
			"Conductor"		 => backend.Conductor,
			"FunkinShader"	  => warped.shaders.FunkinShader,
			"CustomShader"	  => warped.shaders.CustomShader,
			"FunkinText"		=> warped.objects.FunkinText,
			"FlxAnimate"		=> flxanimate.PsychFlxAnimate,
			"FunkinSprite"		=> warped.objects.FunkinSprite,
			"Alphabet"		  => objects.Alphabet,

			"CoolUtil"		  => backend.CoolUtil,
			//"IniUtil"		   => funkin.backend.utils.IniUtil,
			"XMLUtil"		   => warped.backend.XMLHelper,
			...
			"WindowUtils"		=> warped.backend.utils.WindowUtils,
