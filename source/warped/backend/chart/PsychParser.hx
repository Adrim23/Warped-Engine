...
import warped.backend.chart.FNFLegacyParser.SwagSection;
import warped.backend.chart.ChartData.ChartEvent;
import warped.backend.chart.FNFLegacyParser.SwagSong;
	...
	public static function encode(chart:ChartData):Dynamic {
		...
		for (strumLine in chart.strumLines)
			if (strumLine.type == ADDITIONAL && base.gfVersion == null)
				base.gfVersion = strumLine.characters[0];
            if (base.gfVersion == null) base.gfVersion = "gf";
		...
		for (strumLine in chart.strumLines)
			for (note in strumLine.notes) {
                ...
                var chartThing = chart.noteTypes[note.type];
                if (chartThing == null) chartThing = '';
