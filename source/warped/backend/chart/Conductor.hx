...
class Conductor
{
	...
	public static function setupSong(SONG:ChartData) {
		var beatsPM = SONG.meta.beatsPerMeasure;
		var beatsPB = SONG.meta.stepsPerBeat;
		if (beatsPM == null) beatsPM = 4;
		if (beatsPB == null) beatsPB = 4;
		reset();
		mapBPMChanges(SONG);
		changeBPM(SONG.meta.bpm, cast beatsPM, cast beatsPB);
	}
	/**
