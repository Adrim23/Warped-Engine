...
import warped.backend.chart.Conductor;
...
class FNFLegacyParser {
	public static function parse(data:Dynamic, result:ChartData) {
		...
		result.meta.needsVoices = data.needsVoices;
        if (result.meta.needsVoices == null) result.meta.needsVoices = true;

		var camFocusedBF:Bool = false;
		var altAnims:Bool = false;
		var beatsPerMeasure:Null<Float> = data.beatsPerMeasure;
        if (beatsPerMeasure == null) beatsPerMeasure = 4;
	...
	@:noCompletion public static function __convertToSwagSong(chart:ChartData):SwagSong {
		...
		for (strumLine in chart.strumLines)
			switch (strumLine.type) {
				case OPPONENT:
					if (base.player2 == null) base.player2 = strumLine.characters[0];
					if (base.player2 == null) base.player2 = "dad";
				case PLAYER:
					if (base.player1 == null) base.player1 = strumLine.characters[0];
					if (base.player1 == null) base.player1 = "bf";
