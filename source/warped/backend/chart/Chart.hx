...
class Chart {
	...
	public static function loadEventsJson(songName:String) {
		...
		if (FileSystem.exists(path)) {
			try {
				data = Json.parse(File.getContent(path)).events;
			} catch(e) {
	...
	public static function loadChartMeta(songName:String, difficulty:String = "normal", fromMods:Bool = true) {
		...
		data.stepsPerBeat = 4;
		data.needsVoices = true;
		data.icon = "face";
		data.difficulties = [];
		data.coopAllowed = false;
		data.opponentModeAllowed = false;
		data.displayName = data.name;
		data.parsedColor = CoolUtil.getColorFromDynamic(data.color);
		if (data.parsedColor == null) data.parsedColor = defaultColor;

		var jsons:Array<String> = FileSystem.readDirectory(Paths.modFolders(Mods.currentModDirectory+'/songs/${songNameLower}/charts'));
		for (chart in jsons)
			if (!StringTools.endsWith(chart, 'json') || FileSystem.isDirectory(chart)) jsons.remove(chart);
		if (data.difficulties.length <= 0) {
			data.difficulties = [for(f in jsons) if (Path.extension(f = f.toUpperCase()) == "JSON") Path.withoutExtension(f)];
	...
	public static function parse(songName:String, difficulty:String = "normal"):ChartData {
		...
		try {
			if (valid)
				data = Json.parse(File.getContent(chartPath));
		} catch(e) {
