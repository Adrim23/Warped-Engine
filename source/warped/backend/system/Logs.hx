package warped.backend.system;

import flixel.system.debug.log.LogStyle;
import flixel.system.frontEnds.LogFrontEnd;
import haxe.Log;
import warped.backend.utils.NativeAPI;
import warped.backend.utils.NativeAPI.ConsoleColor;
...
class Logs {
	...
	public static function prepareColoredTrace(text:Array<LogText>, level:Level = INFO) {
		...
		var superCoolText = [
			logText('[  '),
			logText('${Std.string(time.getHours())}:${Std.string(time.getMinutes())}:${Std.string(time.getSeconds())}', DARKMAGENTA),
			logText('  |'),
			switch(level) {
