...
import warped.backend.sound.FunkinSound;
...
class DJBoyfriend extends FlxAtlasSprite
{
  ...
  public final playableCharData:PlayerFreeplayDJData;

  public function new(x:Float, y:Float, character:DJCharacter)//? don't pull 2 time here. Use result from above instead!
  {
    ...
    switch (currentState)
    {
      ...
      case Idle:
        ...
        if (getCurrentAnimation() == animPrefix && this.isLoopComplete())
        {
          ...
          {
            playFlashAnimation('Boyfriend DJ', true, false, false);
  ...
  {
    cartoonSnd = FunkinSound.load(getRandomFlashToon(), 1.0, false, true, true, function() {
